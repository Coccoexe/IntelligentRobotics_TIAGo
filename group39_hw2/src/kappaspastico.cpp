...
void TiagoBrain::gr39_task(int id)
{
    ...
    if (id == 1) gr39_move(8.7, -4.2, -180); // Green is special
    ...
    if (id == 1) gr39_move(8.7, -4.2, -270); // Green is special pt.2
...
void TiagoBrain::gr39_pickup(int id)
{
    ...
    if (!found) throw std::runtime_error("Object with ID %d not detected", id);
    ...
    ac2.sendGoal(goal2);
    if (!ac2.waitForResult(ros::Duration(20.0))) throw std::runtime_error("Manipulation server did not finish after 20 seconds");
...
void TiagoBrain::move_head(int a)
{
...
{
    ros::init(argc, argv, "kappaspastico");
    TiagoBrain tb;
    return 0;
}